    ...
    Else If (Message2 = "JOIN"){
		...
		Loop, %UserTabCount% {
			...
			If (ControlAH = ChosenUser) And (Who != WantNick){
				...
				RichEdit_SetSel(Chat%A_Index%, -1,-1)
				CurrTextLen := RichEdit_GetTextLength(Chat%A_Index%)
				RichEdit_SetText(Chat%A_Index%, "==  " Who " has joined!`r`n", "SELECTION", -1)
				RichEdit_SetSel(Chat%A_Index%, CurrTextLen, CurrTextLen + 2)
				RichEdit_SetCharFormat(Chat%A_Index%, "Segoe", "s9", "0xFF0000", "", "Selection")
				RichEdit_SetSel(Chat%A_Index%, -1, -1)
				RichEdit_SetCharFormat(Chat%A_Index%, "Segoe", "s9", "0x000000", "", "Selection")
	...
	Else {
		...
		Loop, %UserTabCount% {
			...
			If (ControlAH = Message3){
				DirNum := A_Index
				If ListChans Not Contains %Message3%
					DirMsg := True
				break
			}
		}
		Gui, 1: Default
		If instr(message, "*") And SubStr(message, instr(message, "*")-1, 1) = A_Space Or instr(message, "*") = 1
			msgbox calc here
		Else If (Message != 0 And !InStr(Message, ServerStatus)) {
...
{
    Global __WSA_UDF, __WSA_ErrMsg, NickName, Channel, Sender, DotText, DOTDOTDOT, CurrTextLen, Chat1, OnlineUsers, WantNick
